...
class NewDevEnvGen(BaseGenerator):
    ...
    def run(self, new_env: str):
        ...
        self.generate_env_config(new_env)
        self.generate_secrets_config_dirs(new_env)

    def generate_env_config(self, new_env: str):
        """
    ...
    def generate_secrets_config_dirs(self, new_env: str):
        ...
        nginx_config_path = self.repo_root / self.SECRETS_CONFIG_RELPATH / new_env / "nginx"
        ...
        for world in self.env_config["world-groups"].enabled_groups:
            ...
            secrets_world_config_path = self.repo_root / self.SECRETS_CONFIG_RELPATH / new_env / "worlds" / world
            ...
            src_server_properties_path = \
                self.repo_root / self.SECRETS_CONFIG_RELPATH / self.env / "worlds" / world / self.WORLDS_CONFIG_DIR / "server.properties"
            dst_server_properties_path = \
                worlds_path / "server.properties"

            if not dst_server_properties_path.exists():
                logger.info(f">> Copying world server.properties from {src_server_properties_path} to {dst_server_properties_path}...")
        ...
        src_default_path = self.repo_root / self.SECRETS_CONFIG_RELPATH / self.env / "worlds" / "default"
        dst_default_path = self.repo_root / self.SECRETS_CONFIG_RELPATH / new_env / "worlds" / "default"
        if not dst_default_path.exists():
            logger.info(f"Copying default config files from {src_default_path} to {dst_default_path}...")
            shutil.copytree(src_default_path, dst_default_path)
        else:
            logger.info(f"Skipped copying files from {src_default_path} to {dst_default_path} as destination directory already existed.")
            logger.info("This script did not validate that the contents of {dst_default_path} was valid - please confirm this manually.")
