...
@validate_access_token
@log_request
def prepare_ws_attach(path: EnvRequestPath):
    """
...
@validate_access_token
@log_request
def up_one_container_handler(path: EnvRequestPath):
    resp = make_cors_response()
    container_name = request.json["container_name"]
...
@validate_access_token
@log_request
def down_one_container_handler(path: EnvRequestPath):
    env = Env(path.env_str)
...
@validate_access_token
@log_request
def restart_one_container_handler(path: EnvRequestPath):
    env = Env(path.env_str)
